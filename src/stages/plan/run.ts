...
import { exemptFixPass, runFixPass } from './fix-pass.js';
...
import { buildSystemContext, writeSystemContext } from '../../core/system-context.js';
...
import { finalizePlan } from './finalize.js';
import type { FinalProjection, RunMode, RunOverrides } from '../../types.js';
...
function buildContract(ctx: RunContext, assessment: string | JsonValue): ReadinessContract {
  return buildReadinessContract({
    ...
    operatorDecisionIds: ctx.readinessProof.operatorDecisionIds,
  });
}
...
async function prepareReadinessContract(
  ...
  if (existsSync(contractFile)) {
    ...
    return {
      ok: false,
      exitCode: 1,
      reason: 'resume requires a current frozen readiness contract',
    };
  } else {
...
function invalidForwardedConfig(message: string): HaltError {
  return new HaltError(message, 1, false, 'config-override-invalid');
}

function invalidSecretHandoff(message: string): HaltError {
  return new HaltError(message, 1, false, 'secrets-handoff-invalid');
}
...
function readForwardedOverrides(env: NodeJS.ProcessEnv, home: string): ResolveOverrides {
  ...
  if (configJson !== undefined && configJson !== '') {
    ...
    try {
      ...
      throw invalidForwardedConfig('AGENT_QUORUM_CONFIG_OVERRIDE_JSON is not valid JSON');
    }
    if (!isJsonObject(parsed)) {
      throw invalidForwardedConfig('AGENT_QUORUM_CONFIG_OVERRIDE_JSON must be a JSON object');
    }
...
function readSecretHandoff(filePath: string, home: string): Secrets {
  ...
  if (!statSync(resolved).isFile()) {
    throw invalidSecretHandoff('AGENT_QUORUM_SECRETS_OVERRIDE_FILE must resolve to a regular file');
  }
  ...
  try {
    ...
    throw invalidSecretHandoff('AGENT_QUORUM_SECRETS_OVERRIDE_FILE is not valid JSON');
  }
  if (!isJsonObject(parsed)) {
    throw invalidSecretHandoff('AGENT_QUORUM_SECRETS_OVERRIDE_FILE must contain a JSON object');
  }
  const token = parsed.telegramBotToken;
  if (token !== undefined && typeof token !== 'string') {
    throw invalidSecretHandoff(
      'AGENT_QUORUM_SECRETS_OVERRIDE_FILE telegramBotToken must be a string',
    );
  }
